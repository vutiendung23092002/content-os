...
- Mỗi draft dùng tối đa 10 ảnh hoặc đúng một video MP4/MOV; không được trộn ảnh và video.
- Video tải trực tiếp bằng signed upload vào private Storage, phải hoàn tất và được kiểm tra metadata trước khi gắn vào draft.
...
5. Với video, gửi signed `file_url` và `description` vào Page `/videos`; Facebook tiếp tục mã hóa video sau khi nhận.
6. On success store remote ID, `published` and sync snapshot.
7. On known rejection set `failed`; on ambiguous timeout set `uncertain` and reconcile before retry.
...
3. Call Meta immediately with `published=false` và `scheduled_publish_time`; bài nhiều ảnh dùng các `media_fbid` trong `attached_media`, video dùng Page `/videos` với signed `file_url`.
...
Luồng hiện tại đăng video thường của Page. Facebook Reel dùng quy trình `video_reels` riêng và chưa được gộp ngầm vào lựa chọn Video.
