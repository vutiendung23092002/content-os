...
const postId =
  "018f0d44-35f0-7b63-99d2-c1b9222cd05d";

const prepared = {
  operationId: "operation-1",
  postId,
  pageId:
    "018f0d44-35f0-7b63-99d2-c1b9222cd06e",
  remotePostId:
...
describe(
  ...
  () => {
    it(
      "updates a remote post message and persists only after Meta succeeds",
      async () => {
        const context = setup();

        await expect(
          context.service.updateMessage(
            postId,
            "Caption mới",
          ),
        ).resolves.toEqual({
        ...
        expect(
          context.client.updatePostMessage,
        ).toHaveBeenCalledWith(
          prepared.remotePostId,
          "Caption mới",
        );
        ...
        expect(
          context.persistence.updateSucceeded,
        ).toHaveBeenCalledWith({
          ...prepared,
          message: "Caption mới",
        });
    ...
    it(
      "removes a scheduled or published remote post and records the previous status",
      async () => {
        const context = setup();

        await expect(
          context.service.remove(postId),
        ).resolves.toEqual({
          operationId: "operation-1",
          postId,
          remotePostId:
        ...
        expect(
          context.client.deletePost,
        ).toHaveBeenCalledWith(
          prepared.remotePostId,
        );

        expect(
          context.client.resolveVideoPostId,
        ).not.toHaveBeenCalled();

        expect(
          context.persistence.removeSucceeded,
        ).toHaveBeenCalledWith({
          ...prepared,
          deletedRemotePostId:
            prepared.remotePostId,
     ...
     */
    it(
      "deletes a legacy video through its associated feed post id",
      async () => {
        const context = setup();
     ...
     */
    it(
      "deletes a canonical video post while preserving its video object id alias",
      async () => {
        const context = setup();
    ...
    it(
      "records a known Meta rejection as failed",
      async () => {
        const context = setup();
        ...
        expect(
          context.persistence.uncertain,
        ).not.toHaveBeenCalled();
      },
    );

    it(
      "marks a retryable remote failure as uncertain and does not persist success",
      async () => {
        const context = setup();
    ...
    it(
      "does not label Meta success as failure when local persistence fails",
      async () => {
        const context = setup();
