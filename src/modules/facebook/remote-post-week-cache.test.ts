...
const weekEnd = new Date("2026-08-23T17:00:00.000Z");
...
function mirrorStore() {
  return {
    replaceWindow: vi
      .fn()
      .mockImplementation(async (input: { posts: RemoteFacebookPost[] }) => ({
...
describe("RemotePostWeekCache", () => {
  it("serves a completed fresh week from Supabase without calling Meta", async () => {
    ...
    const cache = new RemotePostWeekCache(reader, posts, cursors, mirror);
  ...
  it("collapses local and canonical video records with different Facebook ids", async () => {
    ...
    const cache = new RemotePostWeekCache(
      {
        list: vi.fn(),
      },
      posts,
  ...
  it("fetches a cold week, waits for all pages, replaces the complete window and records completion", async () => {
    ...
    expect(mirror.replaceWindow).toHaveBeenCalledTimes(1);
  ...
  it("does not reconcile or mark the week complete when Meta pagination fails", async () => {
    const first = remotePost("post-1", "2026-08-18T02:00:00.000Z");
    ...
    expect(cursors.markSuccess).not.toHaveBeenCalled();
  });

  it("rejects a repeated cursor instead of reconciling an incomplete snapshot", async () => {
    const first = remotePost("post-1", "2026-08-18T02:00:00.000Z");
